...
| `fresh.js` | the page's half of the SW update dance: `reg.update()` on every return to the foreground **and every 10 minutes** (the always-visible shop laptop never fires visibilitychange — that interval is what reaches it, sized to the measured `max-age=600` GH Pages serves), reload on `controllerchange` — silent when just-opened or hidden; mid-work it toasts and **arms a reload for the next time the page goes hidden** (lock, minimise, app switch), so no update ever waits on a human (the owner, 2026-08-01). Once per page life. **Cannot run under `?test=1`** — proven only by a deploy reaching a real phone |
